...
let donePhotoFiles=[];
let ordinarySaveBusy=false,ordinarySaveAttempt=null;
...
async function interventionPhotoSyncState(interventionId){
  ...
  if(ir.data&&Number(ir.data.foto_sincronizzate)!==actual){const u=await syncPhotoMetadata(interventionId);if(u.error)console.warn('Contatore foto non aggiornato:',u.error.message)}
...
async function flushReadyClosureNotifications(interventionId=null){
  ...
  for(const i of data||[]){
    try{const st=await interventionPhotoSyncState(i.id);if(!st.ready)continue;const ok=await notifyAdminClosure('intervention',i.id,st.actual);if(ok){const r=await syncPhotoMetadata(i.id,null,null,true);if(r.error)console.warn('Flag notifica foto non aggiornato:',r.error.message)}}catch(err){console.warn('Notifica intervento pronta non inviata:',err?.message||err)}
  }
}
...
async function setImpersonatedProfile(profileId){
  ...
  await loadAll();
  const forbidden=impersonating()&&['reports','audit','operations'].includes(requestedView);
...
async function loadCompanyArchive(){
  ...
  const [{data:docs,error:docsErr},{data:reads,error:readsErr}]=await Promise.all([
    readAllRows('company_documents',q=>q.order('updated_at',{ascending:false})),
    readAllRows('company_document_reads',q=>q,['document_id','user_id'])
  ]);
...
async function loadSignatureSheets(){
  ...
  const {data,error}=await readAllRows('signature_sheets',q=>q.order('year',{ascending:false}).order('month',{ascending:false}).order('round',{ascending:true}).order('created_at',{ascending:false}));
...
async function readAllRows(table,configure=q=>q,keys=['id']){
  ...
  try{
    for(;;){
      let q=configure(sb.from(table).select('*'));
      for(const key of keys)q=q.order(key,{ascending:true});
      const r=await q.range(offset,offset+499);
      if(r.error)return {data:null,error:r.error};
      const page=r.data||[];
      if(!page.length)return {data:rows,error:null};
      for(const row of page){
        const key=JSON.stringify(keys.map(k=>row[k]));
        if(seen.has(key))return {data:null,error:new Error('I dati di '+table+' sono cambiati durante il caricamento. Aggiorna e riprova.')};
...
async function syncPhotoMetadata(id,status=null,error=null,notified=false){
  const r=await sb.rpc('overgreen_photo_status_v197',{p_id:id,p_status:status,p_error:error,p_notified:notified});
  if(!r.error&&r.data){const local=interventions.find(i=>i.id===id);if(local)Object.assign(local,r.data)}
  return r;
}
async function loadAll(){
  ...
  const [p,s,i,sch,sm,si,e,ew,iw,a,wi,wp,wn,sr,sri,wc,cs,sa,sap]=await Promise.all([
    readAllRows('profiles',q=>q.order('nome')),
    readAllRows('stores',q=>q.eq('attivo',true)),
    readAllRows('interventions',q=>q.order('created_at',{ascending:false})),
    readAllRows('schedules',q=>q.order('giorno')),
    readAllRows('schedule_members',q=>q,['schedule_id','profile_id']),
    readAllRows('schedule_items',q=>q.order('posizione')),
    readAllRows('extras',q=>q.order('giorno_intervento')),
    readAllRows('extra_workers',q=>q,['extra_id','profile_id']),
    readAllRows('intervention_workers',q=>q,['intervention_id','profile_id']),
    readAllRows('attachments',q=>q.order('created_at',{ascending:false})),
    readAllRows('extra_work_items',q=>q.order('posizione')),
    readAllRows('extra_work_item_photos',q=>q.order('created_at')),
    readAllRows('extra_work_item_notes',q=>q.order('created_at')),
    readAllRows('saved_routes',q=>q.order('nome')),
    readAllRows('saved_route_items',q=>q.order('posizione')),
    readAllRows('work_contacts',q=>q.order('nome')),
    readAllRows('contact_stores',q=>q,['contact_id','store_id']),
    readAllRows('schedule_activities',q=>q.order('posizione')),
    readAllRows('schedule_activity_photos',q=>q.order('created_at'))
  ]);
  startupPerf.supabase=performance.now()-supabaseStarted;
  for(const r of [p,s,i,sch,sm,si,e,ew,iw,a,wi,wp,wn,sr,sri,wc,cs,sa,sap])if(r.error)throw r.error;
...
async function runScheduleRollover(force=false){
  ...
  scheduleRolloverPromise=(async()=>{
    ...
    try{
      const roll=await sb.rpc('rollover_overgreen_schedules',{p_today:null});
      startupPerf.rollover=performance.now()-rolloverStarted;renderStartupPerf();
      if(roll.error){
        console.warn('Riporto automatico non riuscito:',roll.error.message);toast('Riporto automatico non riuscito: '+roll.error.message);
        return;
      }
      ...
      for(const result of [sch2,sm2,si2,e2,ew2,sa2])if(result.error)throw result.error;
...
function renderDashboard(){renderDashboardSmart();
  if(typeof renderOperationsShortcut==='function')renderOperationsShortcut();
  if(currentView==='operations'&&typeof renderOperations==='function')renderOperations();
  if(!$('dashToday'))return;
...
const helpPages={
  operations:{title:'Come usare Da gestire',html:`<p>Questa pagina riunisce le azioni da fare sui lavori già registrati.</p><ul><li>Filtra per cliente, ricerca o tipo di azione. Ogni lavoro compare una volta, anche quando ha più segnalazioni.</li><li><strong>Foto e dati</strong> segnala foto attese ma non registrate e target mancanti. Le foto rimaste sul telefono vanno controllate dal dispositivo usato per caricarle.</li><li><strong>Prepara programma</strong> aggiunge la sede alla selezione: scegli data e squadra, poi salva il programma.</li><li><strong>Squadre · 7 giorni</strong> conta ordinari, extra e attività separatamente. Un operatore in più squadre è un invito a verificare gli orari, non la certezza di una sovrapposizione.</li><li>Il PDF apre il programma completo con le date modificabili; non applica i filtri della lista.</li></ul>`},
...
async function createScheduleProgramPdf(from,to){
  if(!window.PDFLib?.PDFDocument)throw new Error('Motore PDF non caricato. Chiudi e riapri l’app e riprova.');
  const groups=schedulePdfGroups(from,to);
  if(!groups.length)throw new Error('Non risultano lavori programmati nel periodo selezionato.');

  const {PDFDocument,StandardFonts,rgb}=window.PDFLib;
  const pdf=await PDFDocument.create();
  const regular=await pdf.embedFont(StandardFonts.Helvetica);
  const bold=await pdf.embedFont(StandardFonts.HelveticaBold);
  const green=rgb(.02,.35,.18),dark=rgb(.08,.18,.13),muted=rgb(.38,.45,.41),line=rgb(.82,.87,.84);
  ...
  const addPage=()=>{
    ...
    y=H-M;
    page.drawText('OVERGREEN',{x:M,y,size:11,font:bold,color:green});
    page.drawText('PROGRAMMA LAVORI',{x:W-M-115,y,size:9,font:bold,color:muted});
    y-=25;
  };
  const ensure=(needed)=>{
    if(!page)addPage();
    if(y-needed<M+25)addPage();
  };
  const linesFor=(value,font,size,maxWidth)=>{
    const safe=pdfSafeText(value);
    if(!safe)return [];
    return wrapPdfText(safe,font,size,maxWidth);
  };
  const drawLines=(value,font,size,x,maxWidth,color=dark,lineHeight=size*1.25)=>{
    const lines=linesFor(value,font,size,maxWidth);
    for(const ln of lines){
      ensure(lineHeight+2);
      page.drawText(pdfSafeText(ln),{x,y,size,font,color});
      y-=lineHeight;
    }
  };

  addPage();
  page.drawText('Programma complessivo',{x:M,y,size:20,font:bold,color:green});y-=26;
  const period=from===to?fmt(from):`${fmt(from)} - ${fmt(to)}`;
  drawLines(period,bold,12,M,CW,dark,15);
  const totalJobs=groups.reduce((n,g)=>n+g.jobs.length,0);
  drawLines(`${totalJobs} lavori programmati`,regular,9,M,CW,muted,12);
  y-=10;

  let previousDay='';
  for(const g of groups){
    const day=String(g.schedule?.giorno||'');
    if(day!==previousDay){
      ensure(42);
      if(previousDay)y-=6;
      page.drawText(pdfSafeText(fmt(day).toUpperCase()),{x:M,y,size:14,font:bold,color:green});
      y-=9;
    ...
    const team=pdfSafeText((g.members||[]).join(' + ')||'Squadra non indicata');
    ensure(30);
    page.drawText(team,{x:M,y,size:11,font:bold,color:dark});y-=16;

    if(g.schedule?.nota_generale){
      drawLines(`Nota giornata: ${g.schedule.nota_generale}`,regular,8.5,M,CW,muted,11);
      y-=3;
    }
    ...
    for(const job of g.jobs){
      ...
      if(job.kind==='ordinary'){
        const st=stores.find(x=>x.id===job.item?.store_id);
        title=`${visualNumber}. ${st?.nome||'Sede'}`;
        sub=[clientLabel(st),st?.citta,st?.indirizzo].filter(Boolean).join(' - ');
        const notes=[];
        if(String(st?.next_visit_note||'').trim())notes.push(`Da fare: ${st.next_visit_note.trim()}`);
        const linked=linkedExtrasForScheduleItem(job.item?.id)||[];
        if(linked.length)notes.push('Extra collegati: '+linked.map(e=>`${e.titolo||'Extra'}${e.numero_target?' #'+e.numero_target:''}`).join('; '));
        detail=notes.join(' | ');
      }else if(job.kind==='extra'){
        const e=job.extra||{},st=stores.find(x=>x.id===e.store_id);
        title=`${visualNumber}. EXTRA - ${e.titolo||'Lavoro extra'}`;
        sub=[clientLabel(e),st?.nome||e.nome_esterno,e.numero_target?`Target/Ticket ${e.numero_target}`:null].filter(Boolean).join(' - ');
        detail=[st?.indirizzo||e.indirizzo_esterno,e.descrizione].filter(Boolean).join(' | ');
      }else{
        const a=job.activity||{},st=stores.find(x=>x.id===a.store_id),meta=activityTypeMeta(a.tipo);
        title=`${visualNumber}. ${(meta?.label||'Attivita').toUpperCase()} - ${a.titolo||meta?.label||'Attivita'}`;
        sub=[a.ora?String(a.ora).slice(0,5):null,st?.nome,a.indirizzo].filter(Boolean).join(' - ');
        detail=a.note||'';
      }

      const titleLines=linesFor(title,bold,10.5,CW-20);
      const subLines=linesFor(sub,regular,8.5,CW-20);
      const detailLines=linesFor(detail,regular,8,CW-20);
      const needed=titleLines.length*13+subLines.length*11+detailLines.length*10+18;
      ensure(Math.max(needed,28));

      for(const ln of titleLines){page.drawText(pdfSafeText(ln),{x:M+8,y,size:10.5,font:bold,color:dark});y-=13}
      for(const ln of subLines){page.drawText(pdfSafeText(ln),{x:M+8,y,size:8.5,font:regular,color:muted});y-=11}
      for(const ln of detailLines){page.drawText(pdfSafeText(ln),{x:M+8,y,size:8,font:regular,color:dark});y-=10}
      y-=7;
    }
    y-=7;
  }
  ...
  for(const p of pdf.getPages()){
    p.drawText(pdfSafeText(`Overgreen ${APP_VERSION} - generato ${generated}`),{x:M,y:17,size:7,font:regular,color:muted});
  }

  pdf.setTitle(pdfSafeText(`Programma Overgreen ${period}`));
  pdf.setAuthor('Overgreen');
  const bytes=await pdf.save();
  const fileName=`Programma Overgreen - ${from===to?from:from+' - '+to}.pdf`;
  const file=new File([bytes],fileName,{type:'application/pdf'});
  if(!file.size)throw new Error('Il PDF generato è vuoto.');
  return {file,fileName,title:'Programma lavori Overgreen',sizeKb:Math.max(1,Math.round(file.size/1024))};
}
...
async function exportScheduleProgramPdf(){
  ...
  }catch(err){const msg='Impossibile creare il PDF programma: '+(err.message||String(err));if(status){status.textContent=msg;status.classList.remove('hidden')}alert(msg);btn.textContent=old}
...
if(typeof bindOperationsUi==='function')bindOperationsUi();
...
async function saveOrdinaryIntervention(continueAnotherDay,btn){
  if(ordinarySaveBusy)return;
  ordinarySaveBusy=true;
  const oldText=btn.textContent,buttons=[$('doneContinueBtn'),$('doneForm').querySelector('[type=submit]')].filter(Boolean);
  buttons.forEach(b=>b.disabled=true);btn.textContent='Salvataggio…';
  try{
    if(!ordinarySaveAttempt){
      ...
      const storeId=$('doneStoreId').value,itemId=$('doneScheduleItemId').value||null;
      const nextNote=continueAnotherDay?'':($('doneHasNextVisitNote').checked?$('doneNextVisitNote').value.trim():'');
      if(!continueAnotherDay&&$('doneHasNextVisitNote').checked&&!nextNote)throw new Error('Scrivi cosa va fatto al prossimo passaggio.');
      if(!$('doneDate').value)throw new Error('Seleziona la data della giornata.');
      const linked=itemId?linkedExtrasForScheduleItem(itemId):[],targets=linked.filter(isEurospinOrdinaryTarget);
      if(!continueAnotherDay&&targets.length&&!confirm('Confermi che in negozio sono stati chiusi tutti i target Eurospin collegati?\n\n'+targets.map(e=>(e.numero_target?'Target '+e.numero_target+' · ':'')+(e.titolo||'Target Eurospin')).join('\n')))return;
    ...
    if(!attempt.args.p_continue){
      try{
        ...
        else if(photoSync?.ready)await flushReadyClosureNotifications(data.id);
      }catch(err){console.warn('Notifica da ritentare:',err)}
    }
    const linkedExtrasToClose=attempt.linkedExtrasToClose,continued=attempt.args.p_continue;
  ...
  finally{ordinarySaveBusy=false;buttons.forEach(b=>b.disabled=false);btn.textContent=oldText}
}
...
if('serviceWorker' in navigator)window.addEventListener('load',()=>navigator.serviceWorker.register('./sw.js?v=200').catch(console.error));
