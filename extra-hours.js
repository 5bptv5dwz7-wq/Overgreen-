/* V201: manual total hours and paginated Eurospin report preview. */
...
const ExtraHoursUi=(()=>{
  ...
  function controls(s){
    ...
    el('extraHoursInput').disabled=!s.loaded||s.saving||!admin();
  ...
  function metadata(s){
    ...
    el('extraHoursMeta').textContent=row?`${row.total_hours==null?'Ore non indicate':`Salvate: ${valueText(row.total_hours)} ore`}${stamp?' · '+stamp:''}`:'Ore non ancora inserite';
  ...
  function close(force=false){
    ...
    if(!force&&s.loaded&&el('extraHoursInput').value!==s.savedText&&!confirm('Chiudere senza salvare le ore modificate?'))return;
  ...
  async function open(extra){
    ...
    const s={extraId:extra.id,row:null,report:null,page:1,zoom:1,loaded:false,saving:false,savedText:'',pdf:null,image:null};state=s;
    ...
    el('extraHoursPreviewStatus').textContent='Caricamento del rapportino…';el('extraHoursCanvas').replaceChildren();tell('');controls(s);
    el('extraHoursDialog').showModal();
    try{
      const [hours,report]=await Promise.all([
      ...
      if(!current(s))return;
      if(hours.error)throw hours.error;if(report.error)throw report.error;
      s.row=hours.data;s.report=report.data;s.loaded=true;s.savedText=valueText(s.row?.total_hours);
      el('extraHoursInput').value=s.savedText;metadata(s);controls(s);await preview(s);
  ...
  async function save(){
    ...
    let hours;try{hours=parseExtraHours(el('extraHoursInput').value)}catch(error){tell(error.message,true);el('extraHoursInput').focus();return}
    s.saving=true;controls(s);tell('Salvataggio…');
    try{
      const {data,error}=await sb.rpc('save_extra_labor_hours_v201',{p_extra_id:s.extraId,p_total_hours:hours,p_expected_revision:s.row?.revision||0,p_source_attachment_id:s.report?.id||null});
      if(!current(s))return;if(error)throw error;
      const row=Array.isArray(data)?data[0]:data;if(!row||row.extra_id!==s.extraId)throw new Error('Salvataggio non confermato. Riprova.');
      s.row=row;s.savedText=valueText(row.total_hours);el('extraHoursInput').value=s.savedText;metadata(s);
      tell(hours==null?'Ore rimosse.':'Ore salvate.');
  ...
  function bind(){
    ...
    el('extraHoursInput').oninput=()=>tell('');
