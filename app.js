...
function renderDashboard(){
  ...
  const todaysItems=scheduleItems.filter(i=>i.stato!=='completato'&&visible.some(s=>s.id===i.schedule_id&&s.giorno===todayStr));
  $('dashToday').textContent=todaysItems.length;
  $('dashPending').textContent=interventions.filter(i=>i.stato==='in_attesa').length+extras.filter(e=>e.stato==='in_attesa').length;
  $('dashDone').textContent=interventions.filter(i=>i.stato==='convalidato'&&i.data_intervento===todayStr).length+extras.filter(e=>e.stato==='completato'&&e.giorno_intervento===todayStr).length;
  ...
  for(const sch of rows){const count=scheduleItems.filter(i=>i.schedule_id===sch.id&&i.stato!=='completato').length;if(!count)continue;const c=document.createElement('article');c.className='card';c.innerHTML=`<strong>${fmt(sch.giorno)}</strong><small>${count} lavori da fare</small>`;c.onclick=()=>setView('schedule');next.appendChild(c)}
}
...
$('loginForm').onsubmit = async (e) => { e.preventDefault(); const b=$('loginForm').querySelector('button[type=submit]'); const box=$('loginError'); box.classList.add('hidden'); box.textContent=''; b.disabled=true; b.textContent='Accesso…'; try { const email=$('loginEmail').value.trim(); const password=$('loginPassword').value; if(!email||!password) throw new Error('Inserisci email e password.'); await signIn(email,password); } catch(err) { console.error(err); box.textContent=err?.message||'Accesso non riuscito.'; box.classList.remove('hidden'); } finally { b.disabled=false; b.textContent='Accedi'; } };
document.querySelectorAll('[data-close]').forEach(b=>b.onclick=()=>closeDialog(b));document.querySelectorAll('[data-view]').forEach(b=>b.onclick=()=>setView(b.dataset.view));document.querySelectorAll('[data-filter]').forEach(b=>b.onclick=()=>{storeFilter=b.dataset.filter;renderStores()});
...
sb.auth.onAuthStateChange(async(_event,s)=>{session=s;if(!s){$('loginScreen').classList.remove('hidden');$('app').classList.add('hidden');return}$('loginScreen').classList.add('hidden');$('app').classList.remove('hidden');try{await loadAll();setView('dashboard')}catch(err){alert('Errore collegamento: '+err.message)}});
