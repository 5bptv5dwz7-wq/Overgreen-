...
function renderDashboard(){
  ...
  if(!admin()){
    const title=next.closest('.panel')?.querySelector('h2');if(title)title.textContent='I tuoi lavori';
    const todayHeading=document.createElement('h3');todayHeading.className='dashboard-section-title';todayHeading.textContent='Da eseguire oggi';next.appendChild(todayHeading);
    let todayCount=0;
    for(const item of todaysItems.sort((a,b)=>(a.posizione||0)-(b.posizione||0))){const st=stores.find(x=>x.id===item.store_id),c=document.createElement('article');c.className='card dashboard-job';c.innerHTML=`<strong>${esc(st?.nome||'Punto vendita')}</strong><small>Intervento ordinario</small><div class="actions"><button class="secondary" data-map>Maps</button><button data-done>Eseguito</button></div>`;c.querySelector('[data-map]').onclick=()=>openAppleMaps(st?.indirizzo,'Eurospin '+(st?.nome||''));c.querySelector('[data-done]').onclick=()=>openDone(st,item.id);next.appendChild(c);todayCount++}
    for(const e of todaysExtras){const st=stores.find(s=>s.id===e.store_id),c=document.createElement('article');c.className='card dashboard-job extra-card';c.innerHTML=`<strong>EXTRA · ${esc(st?.nome||e.nome_esterno||'')}</strong><small>${esc(e.titolo)} · Assegnato a te</small><div class="actions"><button data-open-extra>Apri extra</button></div>`;c.querySelector('[data-open-extra]').onclick=()=>setView('extras');next.appendChild(c);todayCount++}
    if(!todayCount){const empty=document.createElement('p');empty.className='muted';empty.textContent='Nessun lavoro assegnato per oggi.';next.appendChild(empty)}

    const futureHeading=document.createElement('h3');futureHeading.className='dashboard-section-title dashboard-future-title';futureHeading.textContent='Lavori dei prossimi giorni';next.appendChild(futureHeading);
    const futureOrdinary=scheduleItems.filter(i=>effectiveScheduleState(i)==='da_fare').map(i=>({item:i,schedule:visible.find(s=>s.id===i.schedule_id)})).filter(x=>x.schedule&&x.schedule.giorno>todayStr).sort((a,b)=>a.schedule.giorno.localeCompare(b.schedule.giorno)||(a.item.posizione||0)-(b.item.posizione||0));
    const futureExtras=(admin()?extras:extras.filter(e=>myExtraIds.has(e.id))).filter(e=>e.giorno_intervento>todayStr&&['programmato','ricevuto','da_integrare'].includes(e.stato)).sort((a,b)=>a.giorno_intervento.localeCompare(b.giorno_intervento));
    const futureRows=[...futureOrdinary.map(x=>({kind:'ordinary',date:x.schedule.giorno,item:x.item})),...futureExtras.map(e=>({kind:'extra',date:e.giorno_intervento,extra:e}))].sort((a,b)=>a.date.localeCompare(b.date));
    for(const row of futureRows){
      if(row.kind==='ordinary'){
        const st=stores.find(x=>x.id===row.item.store_id),c=document.createElement('article');c.className='card dashboard-job';c.innerHTML=`<strong>${esc(st?.nome||'Punto vendita')}</strong><small>${fmt(row.date)} · Intervento ordinario</small><div class="actions"><button class="secondary" data-map>Maps</button><button class="secondary" data-open-schedule>Apri programma</button></div>`;c.querySelector('[data-map]').onclick=()=>openAppleMaps(st?.indirizzo,'Eurospin '+(st?.nome||''));c.querySelector('[data-open-schedule]').onclick=()=>setView('schedule');next.appendChild(c)
      }else{
        const e=row.extra,st=stores.find(s=>s.id===e.store_id),c=document.createElement('article');c.className='card dashboard-job extra-card';c.innerHTML=`<strong>EXTRA · ${esc(st?.nome||e.nome_esterno||'')}</strong><small>${fmt(row.date)} · ${esc(e.titolo)} · Assegnato a te</small><div class="actions"><button data-open-extra>Apri extra</button></div>`;c.querySelector('[data-open-extra]').onclick=()=>setView('extras');next.appendChild(c)
      }
    }
    if(!futureRows.length){const empty=document.createElement('p');empty.className='muted';empty.textContent='Nessun lavoro assegnato nei prossimi giorni.';next.appendChild(empty)}
    return;
  }
...
function openDone(s,scheduleItemId=''){
  $('doneForm').reset();donePhotoFiles=[];renderDonePhotoSelection();$('doneStoreId').value=s.id;$('doneScheduleItemId').value=scheduleItemId;$('doneTitle').textContent='Intervento · '+s.nome;$('doneDate').value=today();
...
function openHistoryEdit(i){
  historyEditPhotoFiles=[];$('historyEditForm').reset();$('historyEditId').value=i.id;$('historyEditDate').value=i.data_intervento;$('historyEditNotes').value=i.note||'';renderHistoryEditNewPhotos();
...
async function renderPending(){
  ...
  for(const i of p){
    ...
    c.innerHTML=`<div class="pending-review-head"><div><h3>${esc(s?.nome||'Intervento')}</h3><p class="muted">${fmt(i.data_intervento)}</p></div><span class="badge-state">In attesa</span></div><div class="pending-review-section"><strong>Chi ha eseguito</strong><p>${names.length?names.map(esc).join(' · '):'Operatore non indicato'}</p></div><div class="pending-review-section"><strong>Note del dipendente</strong><div class="history-note ${i.note?'':'muted'}">${esc(i.note||'Nessuna nota inserita')}</div></div><div class="pending-review-section"><div class="pending-photo-head"><strong>Foto allegate</strong><span>${pics.length}</span></div><div class="pending-review-photos" data-pending-photos>${pics.length?'<span class="history-loading">Caricamento foto…</span>':'<p class="muted">Nessuna foto allegata.</p>'}</div></div><div class="actions"><button data-ok>Convalida</button><button class="danger-btn" data-no>Rifiuta</button></div>`;
...
function renderSchedules(){
  ...
  for(const s of list){
    ...
    c.innerHTML=`<div class="schedule-card-head"><div><h3>${fmt(s.giorno)}</h3><p class="muted">${esc(members.join(' + '))}</p></div>${admin()?'<div class="actions"><button class="secondary" data-add-stores>Aggiungi punti vendita</button><button class="secondary" data-duplicate>Duplica</button></div>':''}</div><p>${esc(s.nota_generale||'')}</p>`;
    ...
    for(const item of items){if(item.tipo==='ordinario'){
      ...
      const stato=effectiveState==='in_attesa'?'In attesa di convalida':'In programma';
...
async function generateExtraClosurePdf(e,button){
  ...
  try{
    ...
    const fields=[['LAVORO',e.titolo],['LUOGO',place],['INDIRIZZO',address||'Non indicato'],['DATA RICHIESTA',fmt(extraRequestDate(e))],['DATA ESECUZIONE',fmt(e.giorno_intervento)],['OPERATORI',names.join(', ')||'Non indicati'],['STATO','Chiuso e convalidato']];
...
function extraCard(e){
  ...
  c.querySelector('[data-close-extra]')?.addEventListener('click',()=>{if(!e.giorno_intervento)return alert('Prima di chiudere il lavoro inserisci la data di esecuzione da Modifica.');const assigned=extraWorkers.some(w=>w.extra_id===e.id);if(!assigned)return alert('Prima di chiudere il lavoro assegna almeno un dipendente da Modifica.');$('closeExtraId').value=e.id;openDialog('closeExtraDialog')});
...
function openExtraEdit(e){
  $('extraEditId').value=e.id;$('extraEditTitle').value=e.titolo||'';$('extraEditDescription').value=e.descrizione||'';$('extraEditRequestDate').value=extraRequestDate(e)||today();$('extraEditDate').value=e.giorno_intervento||'';
...
$('addScheduleItemsForm').onsubmit=async e=>{e.preventDefault();if(!admin())return;const scheduleId=$('addScheduleId').value,selected=[...$('addScheduleStores').querySelectorAll('input:checked')].map(x=>x.value);if(!selected.length)return alert('Seleziona almeno un punto vendita.');const siblings=scheduleItems.filter(i=>i.schedule_id===scheduleId),maxPosition=siblings.reduce((m,i)=>Math.max(m,Number(i.posizione)||0),0);const {error}=await sb.from('schedule_items').insert(selected.map((store_id,i)=>({schedule_id:scheduleId,tipo:'ordinario',store_id,posizione:maxPosition+i+1,stato:'da_fare'})));if(error)return alert(error.message);$('addScheduleItemsDialog').close();toast(`${selected.length} punti vendita aggiunti`);await loadAll()};
...
$('extraEditForm').onsubmit=async e=>{
  ...
  const payload={store_id:external?null:$('extraEditStore').value,nome_esterno:external?$('extraEditExternalName').value.trim():null,indirizzo_esterno:external?$('extraEditExternalAddress').value.trim():null,titolo:$('extraEditTitle').value.trim(),descrizione:$('extraEditDescription').value.trim()||null,data_richiesta:$('extraEditRequestDate').value,giorno_intervento:$('extraEditDate').value||null};
...
$('closeExtraForm').onsubmit=async e=>{
  ...
  try{
    ...
    for(const [tipo,file] of reports){
      btn.textContent=tipo==='rapportino_eurospin'?'Carico file Eurospin…':'Carico file Overgreen…';
      const safe=(file.name||'rapportino.pdf').replace(/[^a-zA-Z0-9._-]/g,'-');
      ...
      const added=await addAttachment({tipo,extra_id:id,storage_path:path,nome_file:file.name,mime_type:file.type,dimensione_bytes:file.size,caricato_da:profile.id});
