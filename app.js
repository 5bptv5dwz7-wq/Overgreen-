...
async function renderPending(){
  ...
  for(const i of p){
    ...
    c.innerHTML=`<div class="pending-review-head"><div><h3>${esc(s?.nome||'Intervento')}</h3><p class="muted">${fmt(i.data_intervento)} · 🕒 ${esc(closureText(i))}</p></div><span class="badge-state">In attesa</span></div><div class="pending-review-section"><strong>Chi ha eseguito</strong><p>${names.length?names.map(esc).join(' · '):'Operatore non indicato'}</p></div><div class="pending-review-section"><strong>Note del dipendente</strong><div class="history-note ${i.note?'':'muted'}">${esc(i.note||'Nessuna nota inserita')}</div></div><div class="pending-review-section"><div class="pending-photo-head"><strong>Foto allegate</strong><span>${pics.length}</span></div><div class="pending-review-photos" data-pending-photos>${pics.length?'<span class="history-loading">Caricamento foto…</span>':'<p class="muted">Nessuna foto allegata.</p>'}</div></div><div class="actions"><button data-ok>Convalida</button><button class="danger-btn" data-no>Rifiuta</button></div>`;
...
async function generateExtraClosurePdf(e,button){
  ...
  try{
    ...
    const fields=[['LAVORO',e.titolo],['LUOGO',place],['INDIRIZZO',address||'Non indicato'],['DATA RICHIESTA',fmt(extraRequestDate(e))],['DATA ESECUZIONE',fmt(e.giorno_intervento)],['ORARIO CHIUSURA',fmtClosedAt(e.closed_at)],['CHIUSO DA',closedByName(e)],['OPERATORI',names.join(', ')||'Non indicati'],['STATO','Chiuso e convalidato']];
...
$('doneForm').onsubmit=async e=>{e.preventDefault();const btn=e.submitter||$('doneForm').querySelector('[type=submit]');const oldText=btn.textContent;btn.disabled=true;btn.textContent='Salvataggio…';try{const workers=admin()?[...$('doneWorkers').querySelectorAll('input:checked')].map(x=>x.value):[profile.id];if(!workers.length)throw new Error('Seleziona chi ha eseguito.');const files=[...donePhotoFiles];const scheduleItemId=$('doneScheduleItemId').value||null;const linkedToClose=scheduleItemId?linkedExtrasForScheduleItem(scheduleItemId):[];if(scheduleItemId){const localDuplicate=interventions.some(i=>i.schedule_item_id===scheduleItemId&&['in_attesa','convalidato'].includes(i.stato));if(localDuplicate)throw new Error('Questo passaggio è già stato chiuso o è in attesa di convalida.');const {data:existing,error:checkError}=await sb.from('interventions').select('id,stato').eq('schedule_item_id',scheduleItemId).in('stato',['in_attesa','convalidato']).limit(1);if(checkError)throw checkError;if(existing?.length)throw new Error('Questo passaggio è già stato chiuso o è in attesa di convalida.')}const payload={store_id:$('doneStoreId').value,schedule_item_id:scheduleItemId,data_intervento:$('doneDate').value,note:$('doneNotes').value.trim()||null,stato:admin()?'convalidato':'in_attesa',inserito_da:profile.id,convalidato_da:admin()?profile.id:null,convalidato_il:admin()?new Date().toISOString():null,closed_by:profile.id};const {data,error}=await sb.from('interventions').insert(payload).select().single();if(error)throw error;const ir=await sb.from('intervention_workers').insert(workers.map(profile_id=>({intervention_id:data.id,profile_id})));if(ir.error)throw ir.error;if(payload.schedule_item_id){const nextState=admin()?'completato':'in_attesa';const r=await sb.from('schedule_items').update({stato:nextState}).eq('id',payload.schedule_item_id);if(r.error)console.warn('Stato programmazione non aggiornato direttamente:',r.error.message);const localItem=scheduleItems.find(x=>x.id===payload.schedule_item_id);if(localItem)localItem.stato=nextState}if(admin()){const r=await sb.from('stores').update({ultimo_passaggio:payload.data_intervento}).eq('id',payload.store_id);if(r.error)throw r.error}interventions.unshift(data);donePhotoFiles=[];renderDonePhotoSelection();$('doneDialog').close();toast(files.length?`Intervento salvato · ${files.length} foto in caricamento`:admin()?'Intervento convalidato':'Inviato a Lorenzo');renderSchedules();renderDashboard();try{await loadAll()}catch(refreshErr){console.warn('Aggiornamento dati non riuscito dopo il salvataggio:',refreshErr)}if(files.length)enqueueInterventionPhotos(data.id,files).catch(err=>{console.error(err);toast('⚠️ Foto in attesa di sincronizzazione')});if(linkedToClose.length){combinedExtraClosureQueue=linkedToClose.map(x=>({id:x.id}));setTimeout(()=>openNextCombinedExtraClosure(),250)}}catch(err){alert(err.message)}finally{btn.disabled=false;btn.textContent=oldText}};
