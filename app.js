...
function renderDashboardSmart(){const r=$("dashboardSmart");if(!r)return;const h=anomalyCounts(),p=interventions.filter(i=>i.stato==="in_attesa").length+extras.filter(e=>e.stato==="in_attesa").length,o=extras.filter(e=>e.stato!=="completato").length;r.innerHTML=`<button class="smart-chip ${p?"warn":"good"}" data-s="p"><strong>${p}</strong><span>Da convalidare</span></button><button class="smart-chip ${o?"warn":"good"}" data-s="e"><strong>${o}</strong><span>Extra aperti</span></button><button class="smart-chip ${h.total?"alert":"good"}" data-s="h"><strong>${h.total}</strong><span>Anomalie</span></button>`;r.querySelector('[data-s="p"]').onclick=()=>p?openPendingDialog():toast("Nessuna convalida");r.querySelector('[data-s="e"]').onclick=()=>setView("extras");r.querySelector('[data-s="h"]').onclick=()=>setView("settings")}
