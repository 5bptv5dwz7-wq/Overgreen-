...
function renderScheduleHistory(){
  ...
  for(const sch of list){
    const items=scheduleAllItems(sch.id),names=scheduleMemberNames(sch.id),storesIn=items.map(i=>stores.find(st=>st.id===i.store_id)).filter(Boolean);
    const c=document.createElement('article');c.className='card schedule-history-card';
    c.innerHTML=`<div class="schedule-history-head"><div><span class="schedule-date-label">${fmt(sch.giorno)}</span><h3>${esc(names.join(' + ')||'Squadra non indicata')}</h3></div><strong>${items.length} sedi</strong></div>${sch.nota_generale?`<p class="muted">${esc(sch.nota_generale)}</p>`:''}<div class="schedule-history-stores">${storesIn.slice(0,8).map(st=>`<span>${esc(st.nome)}</span>`).join('')}${storesIn.length>8?`<span>+${storesIn.length-8}</span>`:''}</div><div class="actions"><button data-reuse>Riutilizza</button><button class="secondary" data-save-route>Salva come giro</button></div>`;
...
function openReuseScheduleDialog(source){
  if(!admin())return;reuseScheduleSource=source;reuseScheduleSelected=new Set();$('reuseScheduleForm').reset();$('reuseScheduleDate').value=tomorrow();
...
async function createScheduleFromReuse(){
  ...
  const {data,error}=await sb.from('schedules').insert({giorno:date,nota_generale:$('reuseScheduleNote').value.trim()||null,creato_da:profile.id}).select().single();if(error)return alert(error.message);
