...
let session=null,profile=null,profiles=[],managedUsers=[],stores=[],interventions=[],schedules=[],scheduleMembers=[],scheduleItems=[],extras=[],extraWorkers=[],interventionWorkers=[],attachments=[],extraWorkItems=[],extraWorkItemPhotos=[],savedRoutes=[],savedRouteItems=[],signatureSheets=[],auditLogs=[],auditPage=0,auditHasMore=false,companyDocuments=[],companyDocumentReads=[],archiveCategory='modulistica';
...
async function loadAll(){
  ...
  try{
    const [wi,wp]=await Promise.all([sb.from('extra_work_items').select('*').order('posizione'),sb.from('extra_work_item_photos').select('*').order('created_at')]);
    if(wi.error)throw wi.error;if(wp.error)throw wp.error;extraWorkItems=wi.data||[];extraWorkItemPhotos=wp.data||[];
  }catch(workErr){extraWorkItems=[];extraWorkItemPhotos=[];console.warn('Lavorazioni multiple non disponibili:',workErr?.message||workErr)}
...
async function saveExtraPartial(){
  ...
  try{
    ...
    await saveStructuredWorkProgress(id,false,btn);
...
$('closeExtraForm').onsubmit=async e=>{
  ...
  try{
    ...
    await saveStructuredWorkProgress(id,true,btn);
