...
function renderDashboard(){
  ...
  if(!admin()){
    ...
    let todayCount=0;
    for(const item of todaysItems.sort((a,b)=>(a.posizione||0)-(b.posizione||0))){const st=stores.find(x=>x.id===item.store_id),linked=linkedExtrasForScheduleItem(item.id),c=document.createElement('article');c.className='card dashboard-job';c.innerHTML=`<strong>${esc(st?.nome||'Punto vendita')}</strong><small>Intervento ordinario</small>${linked.length?`<div class="linked-extra-reminder"><strong>⚠ Extra da fare insieme</strong>${linked.map(e=>`<span>${esc(e.titolo)}</span>`).join('')}</div>`:''}<div class="actions"><button class="secondary" data-map>Maps</button><button data-done>Eseguito</button></div>`;c.querySelector('[data-map]').onclick=()=>openAppleMaps(st?.indirizzo,'Eurospin '+(st?.nome||''));c.querySelector('[data-done]').onclick=()=>openDone(st,item.id);next.appendChild(c);todayCount++}
    ...
    for(const row of futureRows){
      if(row.kind==='ordinary'){
        const st=stores.find(x=>x.id===row.item.store_id),linked=linkedExtrasForScheduleItem(row.item.id),c=document.createElement('article');c.className='card dashboard-job';c.innerHTML=`<strong>${esc(st?.nome||'Punto vendita')}</strong><small>${fmt(row.date)} · Intervento ordinario</small>${linked.length?`<div class="linked-extra-reminder"><strong>⚠ Extra associato</strong>${linked.map(e=>`<span>${esc(e.titolo)}</span>`).join('')}</div>`:''}<div class="actions"><button class="secondary" data-map>Maps</button><button class="secondary" data-open-schedule>Apri programma</button></div>`;c.querySelector('[data-map]').onclick=()=>openAppleMaps(st?.indirizzo,'Eurospin '+(st?.nome||''));c.querySelector('[data-open-schedule]').onclick=()=>setView('schedule');next.appendChild(c)
      }else{
...
function openDone(s,scheduleItemId=''){
  $('doneForm').reset();donePhotoFiles=[];renderDonePhotoSelection();$('doneStoreId').value=s.id;$('doneScheduleItemId').value=scheduleItemId;$('doneTitle').textContent='Intervento · '+s.nome;$('doneDate').value=today();const linked=scheduleItemId?linkedExtrasForScheduleItem(scheduleItemId):[];$('doneLinkedExtras').innerHTML=linked.length?`<strong>⚠ Ricorda: esegui anche questi extra</strong>${linked.map(e=>`<span>${esc(e.titolo)}</span>`).join('')}`:'';$('doneLinkedExtras').classList.toggle('hidden',!linked.length);
...
function renderSchedules(){
  ...
  for(const s of list){
    ...
    for(const item of items){if(item.tipo==='ordinario'){
      ...
      const stato=effectiveState==='in_attesa'?'In attesa di convalida':'In programma',linked=linkedExtrasForScheduleItem(item.id);
      r.innerHTML=`<div class="schedule-order-line"><strong data-store-detail>${esc(st?.nome||'Punto vendita')}</strong>${admin()?'<div class="order-buttons"><button type="button" class="secondary compact-btn" data-up title="Sposta prima">↑</button><button type="button" class="secondary compact-btn" data-down title="Sposta dopo">↓</button></div>':''}</div><small>${stato}</small>${linked.length?`<div class="linked-extra-reminder"><strong>⚠ Da eseguire insieme all'ordinario</strong>${linked.map(e=>`<span>${esc(e.titolo)}</span>`).join('')}</div>`:''}<div class="actions"><button class="secondary" data-map>Maps</button>${effectiveState==='da_fare'?'<button data-done>Eseguito</button>':''}${admin()&&effectiveState==='da_fare'?'<button class="danger-btn" data-delete-scheduled>Elimina</button>':''}</div>`;
...
$('extraEditForm').onsubmit=async e=>{
  ...
  const payload={store_id:external?null:$('extraEditStore').value,nome_esterno:external?$('extraEditExternalName').value.trim():null,indirizzo_esterno:external?$('extraEditExternalAddress').value.trim():null,titolo:$('extraEditTitle').value.trim(),descrizione:$('extraEditDescription').value.trim()||null,data_richiesta:$('extraEditRequestDate').value,giorno_intervento:$('extraEditDate').value||null,con_ordinario:$('extraEditWithOrdinary').checked};
