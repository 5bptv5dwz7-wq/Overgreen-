...
function renderDashboard(){
  ...
  const todaysItems=scheduleItems.filter(i=>i.stato==='da_fare'&&visible.some(s=>s.id===i.schedule_id&&s.giorno===todayStr));
  ...
  for(const sch of rows){const count=scheduleItems.filter(i=>i.schedule_id===sch.id&&i.stato==='da_fare').length;if(!count)continue;const c=document.createElement('article');c.className='card';c.innerHTML=`<strong>${fmt(sch.giorno)}</strong><small>${count} lavori da fare</small>`;c.onclick=()=>setView('schedule');next.appendChild(c)}
}
...
$('doneForm').onsubmit=async e=>{e.preventDefault();const btn=e.submitter||$('doneForm').querySelector('[type=submit]');const oldText=btn.textContent;btn.disabled=true;btn.textContent='Salvataggio…';try{const workers=[...$('doneWorkers').querySelectorAll('input:checked')].map(x=>x.value);if(!workers.length)throw new Error('Seleziona chi ha eseguito.');const files=[...$('donePhotos').files];const payload={store_id:$('doneStoreId').value,schedule_item_id:$('doneScheduleItemId').value||null,data_intervento:$('doneDate').value,note:$('doneNotes').value.trim()||null,stato:admin()?'convalidato':'in_attesa',inserito_da:profile.id,convalidato_da:admin()?profile.id:null,convalidato_il:admin()?new Date().toISOString():null};const {data,error}=await sb.from('interventions').insert(payload).select().single();if(error)throw error;const ir=await sb.from('intervention_workers').insert(workers.map(profile_id=>({intervention_id:data.id,profile_id})));if(ir.error)throw ir.error;if(payload.schedule_item_id){const nextState=admin()?'completato':'in_attesa';const r=await sb.from('schedule_items').update({stato:nextState}).eq('id',payload.schedule_item_id);if(r.error)throw r.error;const localItem=scheduleItems.find(x=>x.id===payload.schedule_item_id);if(localItem)localItem.stato=nextState}if(admin()){const r=await sb.from('stores').update({ultimo_passaggio:payload.data_intervento}).eq('id',payload.store_id);if(r.error)throw r.error}interventions.unshift(data);$('doneDialog').close();toast(files.length?`Intervento salvato · ${files.length} foto in caricamento`:admin()?'Intervento convalidato':'Inviato a Lorenzo');renderSchedules();renderDashboard();try{await loadAll()}catch(refreshErr){console.warn('Aggiornamento dati non riuscito dopo il salvataggio:',refreshErr)}if(files.length)enqueueInterventionPhotos(data.id,files).catch(err=>{console.error(err);toast('⚠️ Foto in attesa di sincronizzazione')})}catch(err){alert(err.message)}finally{btn.disabled=false;btn.textContent=oldText}};
